package de.vta.vtalauncher.logic.auth;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import de.vta.vtalauncher.exception.vtaAuthenticationException;
import de.vta.vtalauncher.exception.vtaException;
import de.vta.vtalauncher.logic.json.JSONConnector;
...
public class AuthenticationService {
    ...
    /**
     * Stores the current Minecraft path, used for saving the vtalauncher_profiles
     */
    private MinecraftPath mcPath;

    /**
     * Creates a new Authentication Service with the given MinecraftPath.
     * Path is uses to read the vtalauncher_profiles.
     *
     * @param path
     */
     ...
     *
     * @param user the selected user from vtalauncher_profiles or fresh created with password and email
     * @return a valid SessionID or NULL if something went wrong
     */
    public String getSessionID(MinecraftUser user) throws VtaException {
        if (user.hasAccessToken()) {
            if (refresh(user)) {
                LOGGER.info("Login with vtalauncher_profiles successful");
                return user.getSession();
            } else {
                throw new VtaAuthenticationException(VtaAuthenticationException.Reason.USER_CREDENTIALS_ARE_WRONG);
            }
        } else {
            ...
            if (response != null && !response.equals("")) {
                ...
                if (jsonObject.has("selectedProfile")) {
                    ...
                    throw new VtaAuthenticationException(VtaAuthenticationException.Reason.DID_NOT_BUY_MINECRAFT);
                }
            } else {
                throw new VtaAuthenticationException(VtaAuthenticationException.Reason.USER_CREDENTIALS_ARE_WRONG);
            }
     ...
     */
    private String getSSID(String username, String password) throws VtaAuthenticationException {
        if (username != null && !username.equals("") && password != null && !password.equals("")) {
            ...
            try {
                ...
                throw new VtaAuthenticationException(VtaAuthenticationException.Reason.USER_CREDENTIALS_ARE_WRONG, e);
            }
        } else {
            return null;
        }
    ...
    /**
     * Reads the vtalauncher_profiles
     *
    ...
    /**
     * Deletes the vtalauncher_profiles file
     */
    public void deleteProfiles() {
        String path = mcPath.getMinecraftDir() + "vtalauncher_profiles.json";

        LOGGER.info("Trying to delete vtalauncher_profiles! (At: " + path + " )");

        File vtalauncher_profiles = new File(path);

        if (vtalauncher_profiles.exists()) {
            try {
                if (vtalauncher_profiles.delete()) {
                    LOGGER.info("vtalauncher_profiles at: " + path + " deleted!");
                } else {
                    LOGGER.error("Could not delete vtalauncher_profiles at: " + path);
                }
            } catch (Exception e) {
                LOGGER.error("Could not delete vtalauncher_profiles at: " + path, e);
            }
        }
    }
}
