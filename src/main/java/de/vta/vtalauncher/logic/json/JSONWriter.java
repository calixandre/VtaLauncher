package de.vta.vtalauncher.logic.json;Vta Vta
...
public class JSONWriter {
    ...
    public static void saveProfiles(Profiles login) {
        String filename = "vtalauncher_profiles.json";
        JsonWriter writer;

        try {
            if (login.getPath() == null || login.getPath().equals("")) {
                LOGGER.info("Writing vtalauncher_profiles to " + OSHelper.getMinecraftPath());
                writer = new JsonWriter(new FileWriter(OSHelper.getMinecraftPath() + filename));
                login.setPath(OSHelper.getMinecraftPath());
            } else {
                LOGGER.info("Writing vtalauncher_profiles to " + login.getPath());
