...
import de.vta.vtalauncher.exception.*;
import de.vta.vtalauncher.logic.auth.AuthenticationService;
import de.vta.vtalauncher.logic.auth.MinecraftUser;
import de.vta.vtalauncher.logic.auth.Profiles;
import de.vta.vtalauncher.logic.download.DownloadService;
import de.vta.vtalauncher.logic.download.DownloadTasks;
import de.vta.vtalauncher.logic.minecraft.MinecraftInfo;
import de.vta.vtalauncher.logic.minecraft.MinecraftPathImpl;
import de.vta.vtalauncher.logic.minecraft.MinecraftProcess;
import de.vta.vtalauncher.logic.version.MinecraftVersion;
import de.vta.vtalauncher.logic.version.VersionListHelper;
...
public class LogicController {
    ...
    public void init(Config config) {
        ...
        if (config.getVersion() != null) {
            ...
            } catch (vtaVersionNotKnownException e) {
        ...
        if (login != null) {
            LOGGER.info("vtalauncher_profiles.json found! Username is: " + login.getSelectedUser().getUsername());
            ...
            LOGGER.info("No vtalauncher_profiles.json found in " + mMinecraftPath.getMinecraftDir());
        }
    }

    public void setUser(String username, char[] password) throws vtaLogicValueIsNullException {
        ...
        if (username == null || username.equals(" ") || username.equals("")) {
            ...
            throw new vtaLogicValueIsNullException("Username is missing");
        }
        if (pass.equals(" ") || pass.equals("")) {
            LOGGER.error("Password is missing");
            throw new vtaLogicValueIsNullException("Password is missing");
        }
    ...
    public MinecraftUser getUser() throws vtaLogicException {
        if (mProfiles.getSelectedUser() != null) {
            ...
            throw new vtaUserException("No user selected");
        }
    }
    ...
    private void startDownloadService() throws vtaLogicException {
        if (mDownService != null) {
            return;
        }
    ...
    public MinecraftVersion getMinecraftVersion() throws vtaLogicException {
        if (mCurrentVersion == null) {
            throw new vtaLogicException("Current version is null");
        }
    ...
    public void startMinecraft() throws vtaLogicException {
        if (!isMinecraftDownloaded()) {
            ...
            throw new vtaLogicException("Minecraft download not yet complete");
        }

        if (!mProfiles.getSelectedUser().isLoggedIn()) {
            LOGGER.error("Trying to start Minecraft although User is not logged in!");
            throw new vtaLogicException("Trying to start Minecraft although User is not logged in!");
        }
        ...
        if (!process.getSuccess()) {
            ...
            throw new vtaLogicException("Minecraft Process could not be started!");
        } else {
            System.exit(0);
        }
    }
    ...
    public String getMinecraftArgument(String key) throws vtaLogicException {
        ...
        if (argument != null) {
            ...
            throw new vtaLogicException("No Argument for key: " + key);
        }
    }
    ...
    /**
     * Returns a list of users from the current vtalauncher_profiles.
     *
     * @return
     */
